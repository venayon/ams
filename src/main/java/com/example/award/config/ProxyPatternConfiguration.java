 ...
 *    - OldFlow (interface) with a Spring Data implementation (oldFlow bean)
 *    - NewFlow (interface) with a Spring Data implementation (newFlow bean)
 * 
 * 2. AwardRepoProxy implements the OldFlow interface and is marked with @Primary
 *    This means when any component asks for OldFlow, Spring will inject AwardRepoProxy
 * 
 ...
 * 
 * 5. Service Layer (AwardService) simply injects OldFlow:
 ...
 * AwardService
 *     ↓ (injects OldFlow)
 * AwardRepoProxy (@Primary implementation of OldFlow)
 *     ↓ (delegates to)
 * AwardFacade
 *     ↓ (checks feature toggle)
 *     ├─→ OldFlow (@Qualifier("oldFlow"))
 *     └─→ NewFlow (@Qualifier("newFlow"))
 * 
 * Key Benefits:
